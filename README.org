...
* About
  ...
  Wacnet is a BACnet webserver. 
  
  #+ATTR_HTML: style="width: 30em;"
  [[./img/wacnet-print-screen.png]]

  For now it can only be used as a network browser and can't send any
  commands. In other words, it's /read-only/.
...
:  java -jar <filename>
   ...
   For example:
:  java -jar wacnet-0.1.0-BETA-standalone.jar
...
* Not yet implemented - Warnings
  ...
  For now, the application only check the post 47808. The underlying
  code permits it to select other ports, but the configuration page
  isn't completed yet.

* License
  
  Copyright (C) 2013 Frozenlock
